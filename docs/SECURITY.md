...
GitHub-side release controls are configured and verified: environment `npm` exists with only the `v*.*.*` custom tag policy and requires the repository owner as reviewer with self-review permitted for the current single-maintainer repository; active `Protect release tags` and `Protect main` rulesets protect release refs and main against mutation/force updates while requiring the `bootstrap` CI check and pull-request approval. Replace self-review with an independent reviewer when available. npm scope ownership, first-package bootstrap, and the npm trusted-publisher association remain external operator-controlled prerequisites.
