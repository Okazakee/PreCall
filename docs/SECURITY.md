...
The public package is **`@okazakee/precall`**, Apache-2.0, ESM-only, with Node.js >=22.14.0 and Bun >=1.3.14 floors. The public bootstrap prerelease `0.1.0-bootstrap.0` exists; final `0.1.0` is not published. Registry metadata does not verify scope ownership.
