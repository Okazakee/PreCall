...
`bun run release:check` validates metadata, license, version, and optional exact semver tag equality. `bun run release:dry-run` uses one npm-generated candidate for package checks and npm's dry-run publish validation, with no credentials or publication. The tag-only workflow fetches full refs and requires tag commit = checked-out `HEAD` = `origin/main`; it uploads only `candidate.tgz` and `release-manifest.json`, then freshly rechecks source, manifest, and artifact bytes/SHA-512 before publishing without rebuilding. npm `11.14.1` is an exact devDependency resolved in `bun.lock`, invoked via `node_modules/npm/bin/npm-cli.js`; temporary-prefix/global npm installation is not used. The `npm` environment remains associated with the release workflow without manual reviewer approval. Main changes still require a PR and the strict required `bootstrap` status check; deletion/non-fast-forward protection and release-tag protection remain active, while approving reviews and latest-push approval are disabled. Future tag releases publish automatically after validation through OIDC without token fallback.
