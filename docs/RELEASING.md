...
The first public package decisions are settled: PreCall / `precall`, Apache-2.0 (first published as
version 0.1.0), Node.js >=22.14.0, Bun >=1.3.14, and optional LangChain/Resend integrations.
Publication must go through npm Trusted Publishing (GitHub Actions/OIDC), never a token fallback,
and published versions and dist-tags are never rewritten. Current externally published state is
...
The current release is **`precall` 0.2.0**, hosted by the `Okazakee/PreCall` repository and licensed Apache-2.0. The package is ESM-only and declares Node.js >=22.14.0 and Bun >=1.3.14; development remains pinned to `bun@1.3.14`.
...
The npm trusted-publisher association is configured and verified for the release workflow. The bootstrap package remains under the `bootstrap` dist-tag, while `latest` points to stable `0.2.0`; the historical scoped bootstrap package is not mutated.

The `v0.1.0` and `v0.2.0` releases are complete. Future tag releases publish automatically after validation through the protected workflow and npm Trusted Publishing/OIDC, without token fallback or manual GitHub environment approval.
