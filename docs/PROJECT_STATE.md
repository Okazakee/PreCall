...
Owner bootstrap remains before publication: confirm npm scope ownership, configure the npm trusted publisher, then complete first scoped-public package authentication/2FA and push a reviewed `v0.1.0` tag. The `npm` environment and repository rulesets are configured as recorded in `docs/RELEASING.md`; its single-maintainer reviewer policy permits owner self-approval and should be replaced with an independent reviewer when available. No publication, tag, or GitHub Release has been performed.
