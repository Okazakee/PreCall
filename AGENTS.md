...
Stable `precall@0.1.0` remains unpublished; the new unscoped `precall@0.1.0-bootstrap.0` bootstrap is published under `bootstrap`, but npm also assigned it to `latest` and that unintended tag must be corrected. The historical `@okazakee/precall@0.1.0-bootstrap.0` package is registry history only and must not be mutated.
